...
const createUser = asyncHandler(async (req, res) => {
  ...
  const userExistsWithId = await User.findOne({ id });
  const userExistsWithEmail = await User.findOne({ email });
  if (userExistsWithEmail || userExistsWithId) {
    return res
      .status(400)
...
const deleteUserWithId = asyncHandler(async (req, res) => {
  ...
  return res.status(204).json({message: `User with id:${id} deleted successfully`});
});
